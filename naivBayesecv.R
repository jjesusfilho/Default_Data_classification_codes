...
for ( j in 1:numfolds)
{
  ...
  nb.fit <- naiveBayes(default_class ~ LIMIT_BAL + SEX+AGE+ EDUCATION+MARRIAGE+BILL_AMT1+PAY_0+PAY_AMT1+PAY_AMT2+PAY_AMT3+PAY_AMT4+PAY_AMT5+PAY_AMT6, data = x.traincv)
  nb.pred = predict(nb.fit, x.testcv)
  tabl = table(nb.pred, y.testcv)
  mean(nb.pred == y.testcv)
  tp = tabl[2,2]
  fp = tabl[2,1]
  fn = tabl[1,2]
  tn = tabl[1,1]
  ...
  nb.prob = predict(nb.fit, x.testcv,type = "raw")[ ,2]
  pred = prediction(nb.prob,as.numeric(y.testcv))
...
## full model, BAL_LIM1+PAY_0, PAY_O, PAY_2+PAY_0, with no correlated predictors I.E, LIM_BAL + SEX+AGE+ EDUCATION+MARRIAGE+BILL_AMT1+PAY_0+PAY_AMT1+PAY_AMT2+PAY_AMT3+PAY_AMT4+PAY_AMT5+PAY_AMT6
e1 = c(0.73455, 0.82015, 0.8216, 0.81075,  0.4201)#accuracy rate
e2 = c(0.7290352,  0.7129649, 0.6811656,  0.6964565, 0.7133294)#auc
e3 = c( 0.770874, 0.82015, 0.8216,0.81075, 0.2911467)#sensitivity
x = c(1,2,3,4,5)
...
nb.fit <- naiveBayes(default_class ~ ., data = defaultdata, subset = train)
nb.pred = predict(nb.fit, eval.data)
tabl = table(nb.pred, y.eval.data)
mean(nb.pred == y.eval.data)
tp = tabl[2,2]
fp = tabl[2,1]
fn = tabl[1,2]
tn = tabl[1,1]
...
# to create ROC 
nb.prob = predict(nb.fit, eval.data,type = "raw")[ ,2]
nb.pred = prediction(nb.prob,as.numeric(y.eval.data))
nb.perf = performance(nb.pred,"tpr","fpr" )
plot(nb.perf,col=21,lwd=1,add = TRUE)
abline(a=0,b=1,lty=5,col="Gray")

## compute the area under the ROC curve
perf_auc = performance(nb.pred, measure = "auc")
